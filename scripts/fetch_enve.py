...
def fetch_enve(destination: str):
    ...
    if not access_key or not secret_key:
        print("⚠️ Warning: R2/AWS credentials not set in environment.", file=sys.stderr)
        # Attempt fallback to GitHub release if ENVE_TOKEN or GH_TOKEN is present
        token = os.environ.get("ENVE_TOKEN") or os.environ.get("GH_TOKEN") or os.environ.get("GITHUB_TOKEN")
        if token:
            print("Attempting fallback download via GitHub Releases...", file=sys.stderr)
            import subprocess
            res = subprocess.run(
            ...
            if res.returncode == 0:
                subprocess.run(["tar", "-xzf", "/tmp/enve.tar.gz", "-C", os.path.dirname(os.path.abspath(destination))], check=True)
                os.chmod(destination, 0o755)
                print(f"✅ Downloaded enve via GitHub fallback to {destination}")
                return
