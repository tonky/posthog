...
| **7. Live Data Tier Boot & Handshake** | `ci-rust.yml` service boot (`bin/ci-wait-for-docker`): live Redis, Redpanda/Kafka, and ClickHouse queries | **~120s – 180s**<br>(2.0 – 3.0 min;<br>12 min full job) | **1.60s**<br>(boot + live PING<br>+ SQL + cluster ready) | **1.21s**<br>(GHA DAG boot;<br>31s entire gate) | **110x – 148x** | Instant native sockets on localhost; zero Docker bridge network packet virtualization latency. |
